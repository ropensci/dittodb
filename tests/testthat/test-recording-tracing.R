...
with_mock_path(path = file.path(temp_dir, "recording_tracing"), {
  ...
  for (i in seq_along(getAnywhere("dbSendQuery"))) {
    msg <- paste0("No more tracing in ", getAnywhere("dbSendQuery")$where[i])
    test_that(msg, {
      # can't use expect_s4_class because standardGenericWithTrace inherits from
      # standardGeneric
      expect_equivalent(class(getAnywhere("dbSendQuery")[i]), "standardGeneric")
    })
  }
