...
* `db-setup/mariadb-docker.sh` and `db-setup/mariadb-docker-reset.sh` which starts (or stops and then starts for the `*-reset.sh` script) a docker container, installs MariaDB in that container (running on the default port 3306), and loads the correct test user and test data into the database for running tests.
* `db-setup/postgres-docker.sh` and `db-setup/postgres-docker-reset.sh` which starts (or stops and then starts for the `*-reset.sh` script) a docker container, installs Postgres in that container (running on the default port 5432), and loads the correct test user and test data into the database for running tests.
